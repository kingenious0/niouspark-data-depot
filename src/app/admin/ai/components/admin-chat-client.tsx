...
export function AdminChatClient() {
  ...
  // Separate state for UI management
  ...
  // Effect to update local UI state from the server action response
  useEffect(() => {
      if (actionState) {
          ...
          if (actionState.newSession) {
              setChatHistory(prev => [actionState.newSession!, ...prev.filter(s => s.id !== actionState.newSession!.id)]);
          }
      }
  }, [actionState]);
  ...
  const handleSelectChat = (chatId: string) => {
    startHistoryTransition(async () => {
        if (!authToken) return;
        const fetchedMessages = await getChatMessages(chatId, authToken);
                        ...
                        chatHistory.map(session => (
                            ...
                            >
                                <div className='flex flex-col items-start'>
                                    ...
                                    <span className="text-xs text-muted-foreground">{formatDistanceToNow(new Date(session.updatedAt), { addSuffix: true })}</span>
                                </div>
                            </Button>
                        ))
                    )}
           ...
           <div className="flex-shrink-0 border-t bg-background">
            {error && (
                ...
                <Alert variant="destructive">
                    ...
                    <AlertDescription>{error}</AlertDescription>
                      <Button variant="ghost" size="icon" className="absolute top-2 right-2" onClick={() => setError(undefined)}>
                        <X className="h-4 w-4" />
                    </Button>
                </Alert>
                </div>
            ...
            <div className="p-4">
                ...
                <Input
                    type="text"
                    name="message"
                    placeholder={currentChatId ? "Continue this conversation..." : "Ask about sales, customers, revenue..."}
                    className="flex-1"
